...
/** Desktop renders this screen in a wide pane: header actions go inline
 *  (ActionSheetIOS doesn't exist there) and the transcript gets a readable
 *  max column width. */
const DESKTOP = Platform.OS === "macos" || Platform.OS === "windows";

/** Compact icon+label button for the desktop session header. */
function HeaderAction({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={cn(
        "active:opacity-70 h-7 flex-row items-center gap-1 rounded-md border px-2",
        tone === "danger" ? "border-danger/40 bg-danger/10" : "border-border bg-surface",
      )}
    >
      <Ionicons name={icon} size={13} color={tone === "danger" ? COLOR.danger : COLOR.fgMuted} />
      <Text className={cn("text-[11.5px] font-medium", tone === "danger" ? "text-danger" : "text-fg-muted")}>
        {label}
      </Text>
    </Pressable>
  );
...
export default function SessionScreen() {
  ...
  return (
    ...
    >
      ...
      {/* Header — mobile: back chevron + action sheet; desktop: inline buttons. */}
      <View style={{ paddingTop: insets.top }} className="border-b border-border bg-bg-elevated">
        <View className={cn("flex-row items-center gap-2 px-3", DESKTOP ? "py-2" : "pb-2.5 pt-1")}>
          {DESKTOP ? (
          ...
          <View className="flex-1">
            <Text numberOfLines={1} className={cn("font-semibold text-fg", DESKTOP ? "text-[14px]" : "text-[15px]")}>{session.title}</Text>
            <View className="mt-0.5 flex-row items-center gap-2">
              <ActivityDot status={session.activity} size={DESKTOP ? 6 : 7} />
              <Text className={cn("text-fg-muted", DESKTOP ? "text-[11.5px]" : "text-[12px]")}>{ACTIVITY_LABEL[session.activity]}</Text>
              {session.branch ? <Text numberOfLines={1} className="font-mono text-[11px] text-fg-faint">⎇ {session.branch}</Text> : null}
              {DESKTOP ? <Text numberOfLines={1} className="text-[11px] text-fg-faint">· {session.host}</Text> : null}
            </View>
          </View>
          {DESKTOP ? (
            <>
              ...
              {markers.length ? (
                <HeaderAction icon="bookmark-outline" label="Markers" onPress={() => setMarkerSheet(true)} />
              ) : null}
              {session.cwd ? (
                <>
                  <HeaderAction
                    ...
                    label="Changes"
                    onPress={() => router.push(`/changes?id=${session.id}`)}
                  />
                  <HeaderAction
                    icon="terminal-outline"
                    label="Terminal"
                    onPress={() => router.push(`/terminal?id=${session.id}`)}
                  />
                </>
              ) : null}
            </>
      ...
      {/* Composer — shares the transcript's readable column width. */}
      <View style={{ paddingBottom: insets.bottom + 8 }} className="items-center border-t border-border bg-bg-elevated px-3 pt-2">
