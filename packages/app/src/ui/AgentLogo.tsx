/**
 * Real brand logos for coding agents, rendered with react-native-svg (no native
...
function ClaudeMark({ size, color }: MarkProps) {
  return (
...
function CodexMark({ size, color }: MarkProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <Path
        fill={color}
...
function OpenCodeMark({ size, color }: MarkProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <Path fill={color} d="M22 24H2V0h20zM17 4.8H7v14.4h10z" />
    </Svg>
  );
}
...
function LetterMark({ size, color, label }: MarkProps & { label: string }) {
  return (
    <View style={{ width: size, height: size, alignItems: "center", justifyContent: "center" }}>
      <Text style={{ fontSize: size * 0.8, lineHeight: size, fontWeight: "800", color }}>
...
/** Real brand logo for an agent, tinted to `color` (defaults to its brand hue). */
export function AgentLogo({
  ...
  switch (agent) {
    ...
    default:
      return <LetterMark size={size} color={c} label={agentLabel(agent)} />;
  }
}
